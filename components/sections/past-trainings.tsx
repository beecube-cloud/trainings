...
export default function PastTrainings() {
  ...
  const goNext = () => {
    if (selectedIndex !== null) {
      setSelectedIndex((selectedIndex + 1) % trainingImages.length);
    }
  };

  const goPrev = () => {
    if (selectedIndex !== null) {
      setSelectedIndex((selectedIndex - 1 + trainingImages.length) % trainingImages.length);
    }
  };
            ...
            {selectedIndex !== null && (
              <>
                ...
                <div className="relative w-full max-w-4xl max-h-[80vh] aspect-[4/3]">
                  <Image
                    src={trainingImages[selectedIndex].src}
                    alt={trainingImages[selectedIndex].caption}
                    fill
                ...
                <div className="absolute bottom-6 sm:bottom-10 left-0 right-0 text-center">
                  <p className="text-white text-base sm:text-lg font-medium bg-black/40 inline-block px-4 py-2 rounded-full">
                    {trainingImages[selectedIndex].caption}
                  </p>
                </div>
              </>
            )}
